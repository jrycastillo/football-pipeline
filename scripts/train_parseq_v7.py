...
def load_our_crops():
    """Load our 65k auto-labeled crops from manifest CSV."""
    manifest = BASE / "data/annotations/jersey_crops_numbers_only_strict_20260408.csv"
    rows     = list(csv.DictReader(open(manifest)))
    per_class = defaultdict(list)
    for r in rows:
        p    = BASE / r["source_path"]
        jnum = r["jersey_number"].lstrip("0") or "0"
        try:
            n = int(jnum)
        except ValueError:
            continue
        if not (1 <= n <= 99):
            continue
        if p.exists():
            per_class[str(n)].append((str(p), str(n), float(r["confidence"])))

    samples = []
    for label, items in per_class.items():
        # Sort by confidence desc, cap at MAX_PER_CLASS
        items.sort(key=lambda x: -x[2])
        for path, lbl, _ in items[:MAX_PER_CLASS]:
            samples.append((path, lbl))

    unique = len(per_class)
    print(f"  Our crops: {len(samples)} crops, {unique} unique numbers")
    return samples
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output",        default="models/parseq_local_v7.pt")
