...
class JNRService:
    ...
    def predict_batch(self, images, reference_crops=None):
        ...
        for pil_img in processed_imgs:
            ...
            content.append({"type": "text", "text": "Identify the jersey number. JSON: {'number': int|null, 'confidence': float, 'visibility': str}."})
        ...
        while loader.more() and n < MAX_FRAMES:
            ...
            if hasattr(ball_res, "boxes"):
                for b in ball_res.boxes:
                    conf = float(b.conf[0].item())
                    # FILTER: Minimum confidence for ball to avoid false positives
                    if conf < 0.5:
                        continue
                    frame_data["boxes"].append({
                        "xyxy": b.xyxy[0].cpu().numpy().tolist(),
            ...
            for box_data in frame_data["boxes"]:
                    if box_data["id"] is not None and box_data["cls"] == 0: # Only process Players for JNR
                        ...
                        if crop is not None and crop.size > 0:
                               ...
                               # OPTIMIZATION: Variance Filter (Skip Blur) - RELAXED for Re-ID
                               if calculate_laplacian_variance(crop) < 20:
                                   continue
                               
                               # OPTIMIZATION: JNR Stride 3 (Phase 109 - Reduced from 10)
                               # Only send crops to Qwen every 3 frames
                               if n % 3 == 0:
