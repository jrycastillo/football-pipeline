...
class IdentityManager:
    def __init__(self):
        ...
        self.master_registry = {} # {team_jersey_key: original_track_id} - Phase 104
    ...
    def process_detection(self, track_id, detected_number, confidence_str, confidence_val=0.0):
        ...
        # OPTIMIZATION: Temporal Consistency Locking (Phase 100)
        # "If the frame (result) is not changing, lock the jersey number."
        if score > 0.6: # Medium/High only
            ...
            # Lock if consistent for 3 frames
            if count >= 3:
                log(f"🔒 [IdentityManager] TEMPORAL LOCK: Track {track_id} -> Jersey #{detected_number} (3 consecutive hits)")
                self._lock_identity(track_id, detected_number)
    ...
    def resolve_identity(self, current_track_id, jersey_number, team_color):
        # Phase 104: Jersey-Driven Re-Identification
        if jersey_number is None or team_color == "Unknown":
            return current_track_id 

        player_key = f"{team_color}_{jersey_number}"

        # CHECK: Do we already know this player?
        if player_key in self.master_registry:
            original_id = self.master_registry[player_key]

            # If the tracker assigned a NEW ID to a known player
            if current_track_id != original_id:
                # OPTIONAL: Check temporal validity? (e.g. is original_id truly lost?)
                # For "Force Persistence", we assume the Jersey is truth.
                log(f"🔄 [Re-ID] Correction: Track {current_track_id} -> {original_id} (Jersey {player_key})")
                return original_id # FORCE the old ID
        else:
            # First time seeing this jersey. Register it.
            # Only register if confidence is high? (Implicitly trusted if we got here via JNR)
            self.master_registry[player_key] = current_track_id
            log(f"📝 [IdentityManager] Registered {player_key} to Track {current_track_id}")
    ...
    def get_role(self, track_id):
        # Heuristic Role Assignment
        color = self.track_colors.get(track_id, "Unknown")
        
        # 1. Referee (User Specified: Blue)
        if color == "Blue":
            return "Referee"
            
        # 2. Goalkeepers (Heuristic: Unique colors not White/Red)
        if color in ["Green", "Yellow", "Black"]: 
            return "Goalkeeper"
    ...
    def process(self, all_frames, stats_engine, id_manager):
        ...
        for f_idx, frame in enumerate(all_frames):
            ...
            for box in frame["boxes"]:
                # Check for Ball (Class 32=COCO, 1=Sports)
                if box["cls"] in [1, 32]: 
                    ball = box
                    break
...
if __name__ == "__main__":
    ...
    try:
        while loader.more() and n < MAX_FRAMES:
            ...
            # Track
            # Track
            player_res = player_model.track(f, persist=True, tracker="custom_bytetrack.yaml", verbose=False, device=0)[0]
            ball_res = ball_model.track(f, persist=True, tracker="custom_bytetrack.yaml", verbose=False, device=0)[0]
            ...
            if hasattr(player_res, "boxes"):
                for b in player_res.boxes:
                    frame_data["boxes"].append({
                        ...
                        "cls": 0 # Force Class 0 for Players
                    })
            ...
            if hasattr(ball_res, "boxes"):
                for b in ball_res.boxes:
                    frame_data["boxes"].append({
                        "xyxy": b.xyxy[0].cpu().numpy().tolist(),
                        "id": None, # Balls usually don't track well with ID, but if they do, we can use it.
                        "conf": float(b.conf[0].item()),
            ...
            for box_data in frame_data["boxes"]:
                    if box_data["id"] is not None and box_data["cls"] == 0: # Only process Players for JNR
    ...
    raw_tracks, _ = stats_adapter.process_events(all_frames, id_manager)
    ...
    try:
        ...
        # Ensure we wait for it to finish
        subprocess.run(["python3", "tools/merge_identities.py"], check=True)
        log("Entity Resolution Complete. Stats saved to output/final_merged_stats.json.")
    except Exception as e:
        log(f"Entity Resolution Failed: {e}")
    
    # Identify Players (Debug)
    if os.path.exists("output/final_merged_stats.json"):
        with open("output/final_merged_stats.json", "r") as f:
            final_stats = json.load(f)
            log(f"Identify Players: {list(final_stats.keys())}")
    
    print("\n--- Validation Results ---")
    print("Stats Saved to output/final_merged_stats.json.")
